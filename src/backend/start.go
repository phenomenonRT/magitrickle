...
func (a *App) Start(ctx context.Context) (err error) {
	...
	go a.startRouteHealthMonitoring(newCtx)
	defer func() {
		for _, group := range a.ruleSetSnapshot() {
			_ = group.Disable()
		}
...
func (a *App) startRouteHealthMonitoring(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			groups := a.ruleSetSnapshot()
			var wg sync.WaitGroup
			for _, group := range groups {
				wg.Add(1)
				go func(group *RuleSet) {
					defer wg.Done()
					checkCtx, cancel := context.WithTimeout(ctx, 2*time.Second)
					defer cancel()
					if err := group.CheckRouteHealth(checkCtx); err != nil && !errors.Is(err, context.Canceled) && !errors.Is(err, context.DeadlineExceeded) {
						log.Error().Err(err).Str("group", group.IDValue().String()).Msg("failed to check route health")
					}
				}(group)
			}
			wg.Wait()
